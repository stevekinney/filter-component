...
test.describe('editing committed filters', () => {
  ...
  test('changing to a different value shape carries the value into the editor', async ({
    ...
    await filterToken(page, 'Deal value is 42000')
      .getByTitle('Change operator')
      .click();
    ...
    await applyValue(page);
    await expect(
      filterToken(page, 'Deal value between 42000 and 99000'),
    ).toBeVisible();
  });

  test('changing the value pre-fills the editor with the current value', async ({
    page,
  }) => {
    await addSingleValueFilter(page, 'Name', 'contains', 'corp');
    await filterToken(page, 'Name contains corp')
  ...
  test('changing the field restarts operator and value selection', async ({
    page,
  }) => {
    await addSingleValueFilter(page, 'Name', 'contains', 'corp');
    await filterToken(page, 'Name contains corp')
  ...
  test('boolean tokens collapse operator and value edits into one list', async ({
    page,
  }) => {
  ...
  test('editing a multi-select value pre-checks the current selection', async ({
    page,
  }) => {
    ...
    await filterToken(page, 'Stage is any of Lead, Negotiation')
      ...
      .click();
    await expect(
      popover(page).getByRole('option', { name: 'Lead' }),
    ).toHaveAttribute('aria-selected', 'true');
    await expect(
      popover(page).getByRole('option', { name: 'Negotiation' }),
    ).toHaveAttribute('aria-selected', 'true');
    await expect(
      popover(page).getByRole('option', { name: 'Contacted' }),
    ).toHaveAttribute('aria-selected', 'false');
    await pickOption(page, 'Contacted');
    await applyValue(page);
    await expect(
      filterToken(page, 'Stage is any of Lead, Negotiation, Contacted'),
    ).toBeVisible();
  });

  test('removing one enum pill keeps the rest of the filter', async ({
    page,
  }) => {
    await addStageAnyOf(page);
    await page
      .getByRole('button', { name: 'Remove Lead from Stage filter' })
      .click();
    await expect(
      filterToken(page, 'Stage is any of Negotiation'),
    ).toBeVisible();
    await expect(resultCount(page)).toHaveText('2 of 12 deals');
  });

  test('removing the last enum pill removes the whole token', async ({
    page,
  }) => {
    await addStageAnyOf(page);
    await page
  ...
  test('long values truncate in the token but stay fully readable in the editor', async ({
    ...
    const longValue =
      'an extremely long value that cannot possibly fit inside one token';
    await addSingleValueFilter(page, 'Name', 'contains', longValue);
    const valueButton = filterToken(
      page,
      `Name contains ${longValue}`,
    ).getByTitle('Change value');
  ...
  test('cancelling an edit with Escape leaves the token untouched', async ({
    page,
  }) => {
    await addSingleValueFilter(page, 'Name', 'contains', 'corp');
    await filterToken(page, 'Name contains corp')
