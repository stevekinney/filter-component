...
function initialFilterGroup(): FilterGroup {
  const hasInvalidFlag =
    typeof window !== 'undefined' && new URLSearchParams(window.location.search).has('invalid');
...
function ResultCount({ filteredDeals }: { filteredDeals: Deal[] | null }) {
  ...
  return (
    <span className="example-count">
      <strong>{filteredDeals.length}</strong> of <strong>{DEALS.length}</strong> deals
    </span>
  );
}
...
function Application() {
  ...
  const logEvent = (text: string, tone: LogEntry['tone'] = 'neutral') => {
    ...
    setEventLog((entries) =>
      [{ id: ++logIdRef.current, text: `${stamp}  ${text}`, tone }, ...entries].slice(0, 12),
    );
  };
  ...
  const handleFiltersChange = (filters: FilterGroup) => {
    ...
    const count = filters.conditions.reduce(
      (total, member) => total + ('combinator' in member ? member.conditions.length : 1),
      0,
    );
